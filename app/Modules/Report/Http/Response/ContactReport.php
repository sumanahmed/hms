...
class ContactReport
{
    ...
    public function findById($id=null,$start,$end,$group=null)
    {
       ...
       //invoice
        $all[] =  JournalEntry::join("invoices","invoices.id","journal_entries.invoice_id")
                            ...
                            concat("INV-",LPAD(invoices.invoice_number,6,0)) as transectionid,
                            (SELECT group_concat(item.item_name,concat("(",invoice_entries.quantity,IFNULL(item.unit_type,""),")"))
                             FROM invoice_entries join item on item.id =invoice_entries.item_id
                             WHERE invoice_entries.invoice_id = invoices.id) as particularsname ')*/

                           ->selectRaw('journal_entries.*, concat("INV-",LPAD(invoices.invoice_number,6,0)) as transectionid,
                            (SELECT group_concat(item.item_name)
                            FROM invoice_entries join item on item.id = invoice_entries.item_id
                            WHERE invoice_entries.invoice_id = invoices.id) as particularsname,

                            (SELECT  invoices.vat_adjustment) as vat_adjustment,
                              
                            (SELECT  invoices.tax_adjustment) as tax_adjustment,
                            
                            (SELECT  invoices.others_adjustment) as others_adjustment,
                            
                            (SELECT group_concat(invoice_entries.quantity)
                            FROM invoice_entries join item on item.id = invoice_entries.item_id 
                            WHERE invoice_entries.invoice_id = invoices.id) as quantity')
        ...
        $all[] =  JournalEntry::join("invoices","invoices.id","journal_entries.invoice_id")
                            ->selectRaw('journal_entries.*,concat("INV-",LPAD(invoices.invoice_number,6,0)) as transectionid,
                              (SELECT group_concat(item.item_name,concat("(",invoice_entries.quantity,IFNULL(item.unit_type,""),")")) FROM invoice_entries join item on item.id =invoice_entries.item_id WHERE invoice_entries.invoice_id = invoices.id) as particularsname ')
                            ->where('journal_entries.contact_id',$id)
                            ->whereBetween('journal_entries.assign_date',[$start,$end])
                            ->where('journal_entries.account_name_id',21)
                            ->where("journal_entries.jurnal_type",'invoice')
        ...
        $all[] =  JournalEntry::join("payment_receives","payment_receives.id","journal_entries.payment_receives_id")
                                ->selectRaw('journal_entries.*, concat("PR-",LPAD(payment_receives.pr_number,6,0)) as transectionid ,
                                  payment_receives.reference as particularsname,

                                  (SELECT  payment_receives.vat_adjustment) as vat_adjustment,
                                    
                                  (SELECT  payment_receives.tax_adjustment) as tax_adjustment,
                                  
                                  (SELECT  payment_receives.others_adjustment) as others_adjustment')
        ...
        $all[] =  JournalEntry::join("credit_note_refunds","credit_note_refunds.id","journal_entries.credit_note_refunds_id")
                                ...
                                ->selectRaw('IF(journal_entries.jurnal_type=12,\'credit note refund\',\'null\') as jurnal_type,journal_entries.`amount`,journal_entries.debit_credit,journal_entries.assign_date,journal_entries.credit_note_id,journal_entries.credit_note_refunds_id,journal_entries.account_name_id,journal_entries.journal_id,journal_entries.invoice_id,journal_entries.income_id,journal_entries.payment_receives_id,journal_entries.payment_receives_entries_id,journal_entries.contact_id,journal_entries.payment_made_id,journal_entries.payment_made_entry_id, concat("CN-",LPAD(credit_notes.credit_note_number,6,0)) as transectionid ,credit_note_refunds.reference as particularsname')
                                ->where('journal_entries.contact_id',$id)
                                ->where("journal_entries.debit_credit",1)
                                ->whereBetween('journal_entries.assign_date',[$start,$end])
        ...
        $all[] =  JournalEntry::join("bill","bill.id","journal_entries.bill_id")
                                ->selectRaw('journal_entries.*,concat("BILL-",LPAD(bill.bill_number,6,0)) as transectionid,
                                (SELECT group_concat(item.item_name) 
                                FROM bill_entry join item on item.id =bill_entry.item_id 
                                WHERE bill_entry.bill_id = bill.id) as particularsname,
                                
                                (SELECT group_concat(bill_entry.quantity)
                                FROM bill_entry join item on item.id =bill_entry.item_id 
                                WHERE bill_entry.bill_id = bill.id) as quantity')
        ...
        $all[]=  JournalEntry::join("invoices","invoices.id","journal_entries.invoice_id")
                                ->selectRaw('journal_entries.*, concat("INV-",LPAD(invoices.invoice_number,6,0)) as transectionid,                                  
                                "Sales Commission" as particularsname')
    ...
    public function openningBalance($id=null,$start)
    {
        ...
        //calculating adjustments in credit
        ...
        // $all['invoice_cr'] = (double)($all['invoice_cr'][0]->total);

        //disabling adjustment calculation for credit
        $all['invoice_cr'] = 0;
        ...
        $all['payment_recieve_cr'] =  JournalEntry::join("payment_receives","payment_receives.id","journal_entries.payment_receives_id")
                                      ...
                                      ->with('paymentReceive')
                                      ->orderBy("journal_entries.assign_date",'asc')
                                      ->selectRaw("sum(journal_entries.amount + IF(payment_receives.vat_adjustment REGEXP '^[0-9]+$', payment_receives.vat_adjustment, 0) + + IF(payment_receives.tax_adjustment REGEXP '^[0-9]+$', payment_receives.tax_adjustment, 0) + + IF(payment_receives.others_adjustment REGEXP '^[0-9]+$', payment_receives.others_adjustment, 0)) as total")
                                      ->get();

        $all['payment_recieve_cr'] = (double)($all['payment_recieve_cr'][0]->total);
