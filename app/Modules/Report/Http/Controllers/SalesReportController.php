...
class SalesReportController extends Controller
{
    public function index(){
        ...
        return view("report::sales.index",compact('OrganizationProfile','companys','products','roads','start','end'));
    }
    ...
    public function filter(Request $request){
        ...
        $final_sales     = FinalSales::join('final_sales_entries','final_sales_entries.final_sales_id','sales_summary.id')
                                        ...
                                        ->when($product_id != 0, function ($query) use ($product_id) {
                                            return $query->where('final_sales_entries.product_id', $product_id);
                                        })
                                        ->when($road_id != 0, function ($query) use ($road_id) {
                                            return $query->where('sales_summary.road_id', $road_id);
                                        })
                                        ->select('sales_summary.date','sales_summary.invoice_number','item.product_code','item.item_name','item.item_sales_rate','item.item_purchase_rate','final_sales_entries.quantity')
        ...
        return view("report::sales.index",compact('OrganizationProfile','companys','products','roads','items','company_name','company_id','start','end','report_type'));
    }
}
