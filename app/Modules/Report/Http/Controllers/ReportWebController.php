...
class ReportWebController extends Controller
{
    ...
    public function accountTransactions()
    {
        ...
        $JournalEntry       =  JournalEntry::whereBetween('assign_date',[$start,$end])->get()->sortBy('id');
    ...
    public function accountTransactionsSearch(Request $request)
    {
        ...
        {
            $JournalEntry =  JournalEntry::whereBetween('assign_date',[$start,$end])->get()->sortBy('assign_date')->sortBy('assign_date');
    ...
    public function ProfitLoss()
    {
        ...
        $JournalEntry=$this->checkBranch($JournalEntry);
        $branch=Branch::all();
        return view('report::profit_loss',compact('JournalEntry','start','end','accounts','OrganizationProfile','branch'))->with('branch_id',$this->branch_id);
    }
    ...
    public function BalanceAndSheet()
    {
        ...
        $TotalretainedEarnings= $retainedEarnings->netprofit();
      ...
      return view('report::BalanceSheet.index',compact('currentYearEarning','longTermLibilities','currentLibilities','FixedAsset','stock','bank','others_current_asset' ,'others_asset','cash','current_asset','start','end','accounts','OrganizationProfile','Totalnetprofit','TotalretainedEarnings'))->with('branch_id',$this->branch_id);
    }
    ...
    public function item()
    {
        ...
        return view('report::item', compact('subcategories', 'item', 'start', 'end', 'OrganizationProfile'));
    }
    ...
    public function itemDetails($id,$start,$end)
    {
        ...
        $bills = Bill::join('bill_entry','bill_entry.bill_id','bill.id')
                    ->selectRaw('bill_entry.rate as rate')
                    ->get();
        $OrganizationProfile = OrganizationProfile::find(1);
        $current_time = Carbon::now()->toDayDateTimeString();
        ...
        return view('report::item_details',compact('subcategories','stock', 'bills','InvoiceEntry','start','end','item_report','item','OrganizationProfile'));
    }
    ...
    public function cashbook()
    {
        ...
        $start = (new DateTime($current_time))->modify('-0 day')->format('Y-m-d');
        $end = (new DateTime($current_time))->modify('+0 day')->format('Y-m-d');
        $yesterday= (new DateTime($current_time))->modify('-1 day')->format('Y-m-d');
    ...
    public function cashbooksearch(Request $request)
    {
      ...
      //  $account_type = Account::all();
        ...
        foreach ($account_type as $key => $value) {
            ...
            $start = date("Y-m-d",strtotime($request->input('from_date')."-1 day")).' '.'23:59:59';
            $cash_inhand_debit = JournalEntry::select(DB::raw('SUM(amount) as cash_inhand'))->whereBetween('assign_date',[$begin_time,$yesterday])->where('account_name_id',$value->id)->where('debit_credit',1)->get()->sortBy('assign_date');
            $cash_inhand_credit = JournalEntry::select(DB::raw('SUM(amount) as cash_inhand'))->whereBetween('assign_date',[$begin_time,$yesterday])->where('account_name_id',$value->id)->where('debit_credit',0)->get()->sortBy('assign_date');
        ...
        //$JournalEntrys=JournalEntry::all()->sortBy('assign_date')->sortBy('jurnal_type');

        $OrganizationProfile = OrganizationProfile::find(1);
        $start = date("Y-m-d",strtotime($request->input('from_date'))); //$start changes as it is used in from to display
        $end = date("Y-m-d",strtotime($request->input('to_date')."+0 day")); // $end changes as it is used in from to display
        return view('report::cash_book',compact('JournalEntrys','start','end','accounts','OrganizationProfile','total_cash_inhand','current_cash_in_hand','total_current_cash_inhand_debit','total_current_cash_inhand_credit','total_cash_inhand_debit','total_cash_inhand_credit'))->with('branch_id',$this->branch_id);
    }
