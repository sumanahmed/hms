...
@endphp
@section('title'," $display_name Sales Commission".date("Y-m-d h-i-sa"))
                        ...
                        <div class="md-card-content invoice_content print_bg" style="height: 100%;">
                            ...
                            <div class="uk-grid" data-uk-grid-margin="">
                                ...
                                <div class="uk-width-small-5-5 uk-text-center">
                                    ...
                                    <p style="line-height: 5px;" class="heading_b">{{ $ag->display_name }} Sales Commission</p>
                                ...
                                <div class="uk-width-1-1">
                                    <table class="uk-table">
                                        ...
                                        <tr class="uk-text-upper">
                                            ...
                                            <th class="uk-text-right" >Total <i class="material-icons" style="font-size: larger; color: orangered">trending_flat</i></th>
                                            <th class="uk-text-right" id="salesamount"> </th>
                                            <th class="uk-text-right" id="totalrecieve"> </th>
                                            <th class="uk-text-right" id="total_payable"></th>
                                            <th class="uk-text-right" id="paid"></th>
                                            <th class="uk-text-right" id="balance"></th>
                                        </tr>
                                        </tfoot>
                                        <tbody id="single_agent">
                                            ...
                                            <tr class="uk-text-upper">
                                               ...
                                               <td class="uk-text-right">{{ $opening_balance_invoices[0]->invoice_total_amount?number_format($opening_balance_invoices[0]->invoice_total_amount, 2, '.', ''): ''  }}</td>
                                               <td class="uk-text-right">{{ $opening_balance_invoices[0]->invoice_due_amount?number_format($opening_balance_invoices[0]->invoice_total_amount - $opening_balance_invoices[0]->invoice_due_amount, 2, '.', ''):''  }}</td>
                                               <td class="uk-text-right">{{ $opening_balance_debit?number_format($opening_balance_debit, 2, '.', ''):''  }}</td>
                                               <td class="uk-text-right">{{ $opening_balance_credit?number_format($opening_balance_credit, 2, '.', ''):'' }}</td>
                                               <td class="uk-text-right">Balance</td>
                                           </tr>

                                       @foreach($journal as $value)
                                           <tr class="uk-text-upper">
                                               <td class="uk-text-left">{{ date('d-m-Y',strtotime($value->assign_date)) }} </td>
                                               <td class="uk-text-left">{{ $value->invoice_id? "INV-".str_pad($value->invoice->invoice_number,6,'0',STR_PAD_LEFT): "  SC-".str_pad($value->SalesCommission->scNumber,6,'0',STR_PAD_LEFT) }} </td>
                                               <td class="uk-text-right">{{ $value->invoice_id?number_format($value->invoice->total_amount, 2, '.', ''):''  }}</td>
                                               <td class="uk-text-right">{{ $value->invoice_id?number_format($value->invoice->total_amount - $value->invoice->due_amount, 2, '.', ''):''  }}</td>
                                               <td class="uk-text-right">{{ $value->invoice_id?number_format($value->amount, 2, '.', ''):''  }}</td>
                                               <td class="uk-text-right">{{ $value->salesComission_id?number_format($value->SalesCommission->amount, 2, '.', ''):'' }}</td>
                                               <td class="uk-text-right">Balance</td>
                                           </tr>
                                       @endforeach
                                        </tbody>
