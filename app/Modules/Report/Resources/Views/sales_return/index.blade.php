                ...
                <div class="md-card md-card-single main-print">
                    <div id="invoice_preview hidden-print">
                        <div class="md-card-content invoice_content print_bg" style="height: 100%;">
                            {!! Form::open(['url' => route('sales_return_report_filter'), 'method' => 'post', 'class' => 'user_edit_form', 'id' => 'my_profile', 'files' => 'true', 'enctype' => "multipart/form-data", 'novalidate']) !!}
                            ...
                            <div class="uk-grid" data-uk-grid-margin="">
                                <div class="uk-width-small-5-5 uk-text-center">
                                    ...
                                    <p style="line-height: 5px; margin-top: 35px;"class="uk-text-large">{{ $OrganizationProfile->company_name }}</p>
                                     <p style="line-height: 5px; margin-top: 18px;"class="uk-text-large">Sales Return Report</p>
                                        ...
                                        <div class="uk-width-1-1">
                                            ...
                                            <table class="uk-table">
                                                ...
                                                <tr class="uk-text-upper">
                                                    ...
                                                    <th class="uk-text-left">Company</th>
                                                    <th class="uk-text-left">Item Code</th>
                                                    <th class="uk-text-left">Item Name</th>
                                                    <th class="uk-text-left">Return Quantity</th>
                                                    <th class="uk-text-left">SR Rate</th>
                                                    <th class="uk-text-left">SR Amount</th>
                                                </tr>
                                                </thead>
                                                <tbody>
                                                <?php $i=1; $total_return_quantity = 0; $total_return_amount = 0;?>
                                                @foreach($item as $value)
                                                    <tr class="uk-table-middle">
                                                        <td>{{ $i }}</td>
                                                        <td>{{ $value['date'] }}</td>
                                                        <td>{{ $helper->getCompanyname($value['company_id']) }}</td>
                                                        <td>{{ $value['product_code'] }}</td>
                                                        <td>{{ $value['item_name'] }}</td>
                                                        <td>{{ $value['quantity'] }}</td>
                                                        <td>{{ $value['item_sales_rate'] }}</td>
                                                        <td>{{ $value['item_sales_rate'] * $value['quantity'] }}</td>
                                                    </tr>
                                                    <?php $i++; ?>
                                                    @php
                                                        $total_return_quantity          += $value['quantity'];
                                                        $total_return_amount            += $value['item_sales_rate'] * $value['quantity'];
                                                    @endphp
                                                @endforeach
                                                </tbody>
                                                <tr class="uk-table-middle">
