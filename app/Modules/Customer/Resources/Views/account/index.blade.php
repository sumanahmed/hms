                        ...
                        <div class="md-card-content">
                            <table  class="uk-table css-serial" cellspacing="0" width="100%" id="data_table" >
                                ...
                                <tr>
                                    ...
                                    <th  width="20%" class="uk-text-center">Transaction ID</th>
                                    <th width="15%" class="uk-text-center">DATE</th>
                                    ...
                                    <th  width="20%" class="uk-text-center">Expense Type</th>
                                ...
                                <tbody id="expense">
                                    ...
                                    @foreach ($recruitexpensepax as $value)
                                        ...
                                        @if($value->RecruiteExpense->expense_id)
                                            ...
                                            @if(isset($value->RecruiteExpense->expense->amount))
                                                <tr>
                                                    <td width="10%" class="uk-text-center"></td>
                                                    @php
                                                        if ($value->expense_id != null){
                                                        $type = 'Expense';

                                                        $transaction = 'EXP-'.$value->RecruiteExpense->expense->expense_number;
                                                        ...
                                                        $transaction = 'SC-'.$value->RecruiteExpense->salesCommission->expense_number;
                                                        }

                                                    @endphp
                                                    <td width="20%" class="uk-text-center">{{ $transaction }}</td>
                                                    <td width="15" class="uk-text-center">{{ date("d-m-Y", strtotime($value->created_at)) }}</td>
                                                    <td width="20%" class="uk-text-center">{{ $value->RecruiteExpense->Sector->title }}</td>
                                                    <td width="20%" class="uk-text-center">{{ $con->getCustomerName($value->RecruiteExpense->expense->vendor_id) }}</td>
                                                    <td width="20%" class="uk-text-center">{{ round($value->RecruiteExpense->expense->amount*$var2/$var1,3) }}</td>

                                                    <?php  
                                                    
                                                    $all_expense_total += round($value->RecruiteExpense->expense->amount*$var2/$var1,3) ;
                                                    ?>

                                                    <td width="20%" class="uk-text-center">{{ $type }}</td>

                                                    <td width="15%" class="uk-text-center">
                                                        <a href="{{ route('expense_show', ['id' =>  $value->RecruiteExpense->id]) }}"><i data-uk-tooltip="{pos:'top'}" title="View" class="md-icon material-icons">visibility</i></a>
                                                    </td>
                                                </tr>
                                             @endif
...
<option style="display: none;" id="taotalExpenseCount" class="hidden"> {{ $all_expense_total }}</option>
    ...
    <script src="{{ url('admin/assets/js/pages/kendoui.min.js')}}"></script>
      ...
      <script>
         function totalExpense()
         {
             ...
             var expense_num = document.getElementById("taotalExpenseCount").innerText;
             ...
             // }
             document.getElementById("expense_num").innerText = expense_num;
